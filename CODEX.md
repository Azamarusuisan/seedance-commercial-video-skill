...
11. **最優先・スキャフォールド完了/実行が唯一の残タスク: リップスティックCMのリトライ。** ユーザーがこの作業をCodexに依頼済み。Codex側で`workspace/projects/lipstick-cm-30s/shots/*/`と`storyboard/00〜06`の企画・承認ゲート定義は既に完成している(2026-07-01、Claude Codeが棚卸し確認済み)。**残っているのは4枚の写実キービジュアルを実際に生成することだけ。**
    ...
    **2026-07-01追加(Claude Code、要確認・未修正のまま残した点): `workspace/projects/macneo-pc-cm-15s/storyboard-structure.md`の「Model target: Seedance 2 via Palmier MCP」という記述は、§2で確定した「Seedance生成は全てHiggsfield MCP経由、Palmier Proは仕上げ専用」という方針と矛盾しているように見える。同じ`project-state.json`の`jobs[0]`は`prompt_path`/`asset_manifest`/`permission_manifest`という、他プロジェクトと同じHiggsfield MCPリクエスト準備パターンのフィールド構成になっており、Palmier Pro固有のフィールド(`referenceMediaRefs`等)は使っていない。おそらく「素材をPalmierのメディアライブラリにインポート済み」という事実(`primary_reference: "Palmier media B7B16D97 / hero.webp"`)との混同で、実際の生成ルート変更を意味していないと推測するが、断定はしていない。プロンプト内の"electric arcs"/"electric energy"（FP-003）は同ファイル内で既に修正済み。この「Palmier MCP」という記述自体は書き換えていない — Codex側で意図を確認してから修正すること(意図的な方針転換であれば、CODEX.md §2の記述もセットで更新が必要)。**
