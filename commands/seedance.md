...
Before task actions, read `skills/seedance/SKILL.md` and the referenced workflow files under `skills/seedance/references/`. Follow the use-case router, image-to-video handoff, TikTok storyboard/cast-library route, rights gate, budget lock, retry strategy, acceptance criteria, and delivery package rules.
...
2. If the brief is incomplete, collect only missing essentials: video use case, project/name, pronunciation if relevant, audience, placement, duration, aspect ratio, count, reference assets, audio policy, storyboard panel count for narrative work, budget, job limit, and retry limit.
