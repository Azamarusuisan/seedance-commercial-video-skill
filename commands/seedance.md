...
Before task actions, read `skills/seedance/SKILL.md` and the referenced workflow files under `skills/seedance/references/`. Follow the use-case router, image-to-video handoff, TikTok storyboard/cast-library route, demand-pattern route, TikTok ad-ops route, rights gate, budget lock, retry strategy, acceptance criteria, and delivery package rules.
...
9. For TikTok Ads work, stop before publishing or account operations; produce checks, scripts, prompts, A/B plans, and human approval gates only.
