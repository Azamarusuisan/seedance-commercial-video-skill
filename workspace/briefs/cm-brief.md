...
- Aspect ratios: 9:16 vertical first, 16:9 landscape as a secondary embed/portfolio version
- Duration: 15秒想定
- Output count: proposal is 2 versions total, but generate one at a time after approval
- Retry ceiling: 2
...
- 主用途: Xのタイムライン、ショート動画、スマホ閲覧。
- 狙い: 冒頭3秒でスクロールを止める。
- 構図: 中央に馬のシルエット、上部に短いコピー、下部に「フジ AI開発」。
- 注意: 画面内テキストは最小限。細かい説明はnote本文に逃がす。

### 16:9 横動画

- 主用途: note記事内の埋め込み、YouTube風プレビュー、LP/ポートフォリオ。
- 狙い: 制作実績として見せやすいワイドな世界観にする。
- 構図: 左から右へ流れる光のレーストラック、中央に馬、左右にAI分析パネル。
- 注意: 縦用プロンプトをそのまま横へ流用しない。横画面では左右の余白と奥行きを使う。

## 映像構成

1. 0-3秒: 未来的な競馬場と光のトラック。大きく「競馬AI」。
2. 3-7秒: AIデータパネル、波形、レース分析の抽象表示が馬のシルエットを囲む。
3. 7-11秒: 利用シーンとして、Xで見た人が思わず止まるようなCM表現。競馬データが美しい映像へ変わる。
4. 11-15秒: 「フジ AI開発」を強く見せて締める。

9:16では縦方向の奥行きと中央配置を優先する。16:9では左右に広がるAIパネルとワイドなレーストラックを優先する。
...
- 予算上限: TBD
- 予算が不明な場合の最小構成: まず9:16、15秒、1080p、音声なし、1本。
- 横版も作る場合は、縦版を確認してから16:9を1本だけ生成する。
- 再生成は各フォーマット最大2回まで。ただしクレジット不明時は縦1本を優先する。
