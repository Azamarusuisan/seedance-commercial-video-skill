...
- Review the current 60-second script from `generation-state.json` / `script.beats[]`.
...
```text
http://127.0.0.1:8787/workspace/ui/script.html
```

The script page renders `generation-state.json` -> `script.beats[]`, including visual action, Japanese narration, and planned telop for each beat.
